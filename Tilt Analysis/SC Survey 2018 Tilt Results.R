...
res <- read.table("U:/Snow Crab/Stock Assessment 2019/SC Survey 2018 Tilt Results.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
...
s$bottom.temperature.minilog <- NA
s$bottom.temperature.star.oddi.headline <- NA
s$bottom.temperature.star.oddi.footrope <- NA
s$bottom.temperature.minilog.slope <- NA
s$bottom.temperature.star.oddi.headline.slope <- NA
s$bottom.temperature.star.oddi.footrope.slope <- NA

t <- start.time(s)

# Bias between old and new:
dbarplot(table(as.numeric(t-t.old)), width = 1)
vline(0, lwd = 2, col = "red")
mean(as.numeric(t.old - t))
...
grid()
for (i in 1:nrow(s)){
   e <- read.esonar(year = 2018, tow.id = s$tow.id[i], type = "depth") 
...
clg()
plot(c(30, 120), c(0, 15), type = "n", xaxs = "i", yaxs = "i", xlab = "Depth(m)", ylab = "Wing sspread(m)")
grid()
for (i in 1:nrow(s)){
   e <- read.esonar(year = 2018, tow.id = s$tow.id[i], type = "depth") 
   e$time <- time2min(time(e), start.time(s[i, ]))
...
for (i in 111:120){
   e <- read.esonar(year = 2018, tow.id = s$tow.id[i], type = "depth") 
   h <- read.star.oddi(year = 2018, tow.id = s$tow.id[i], type = "depth") 
...
for (i in 1:nrow(s)){
   ...
   h <- read.star.oddi(year = 2018, tow.id = s$tow.id[i], type = "depth") 
  # f <- read.star.oddi(year = 2018, tow.id = s$tow.id[i], type = "tilt")
   m <- read.minilog(year = 2018, tow.id = s$tow.id[i])
...
grid()
image(as.numeric(rownames(m)), as.numeric(colnames(m)), m, col = grey(seq(1, 0, len = 100)), add = TRUE,
      xlab = "", ylab = "", xlim = c(-2, 7), xaxs = "i", ylim = c(0, 5), zlim = c(0, 0.15),  yaxs = "i")
mtext("Time (min)", 1, 2.25, cex = 1.5)
