...
Type objective_function<Type>::operator()(){
 ...
 //  DATA_MATRIX(z);        // Comparative survey catch observations.
   ...
   // Export results:
   ADREPORT(swept_area);
   ADREPORT(swept_area_passive);
   //REPORT(mean_width);
   //REPORT(mean_width_passive);
      
   // Binomial model and likelihood:
   //PARAMETER(alpha_z);
   //vector<Type> eta(n_comparative);
   //vector<Type> p_eta(n_comparative);
   //for (int j = 0; j < n_comparative; j++){
   //   eta[j] = alpha_z + log(swept_area[n_station - n_comparative + j]) - log(swept_area[n_station + j]); // +
               // beta_av * log(swept_area_passive[n_station - n_comparative + j]) - beta_jm * log(swept_area_passive[n_station + j]); 
  //    p_eta[j] = Type(1) / (Type(1) + exp(-eta[j]));
  //    res -= dbinom(z(j,0), z(j,0) + z(j,1), p_eta[j], true)
  // }
