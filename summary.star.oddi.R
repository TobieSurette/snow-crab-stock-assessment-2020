summary.star.oddi <- function(x, truncate = TRUE, round = TRUE, ...){
   # SUMMARY.STAR.ODDI - Returns summary statistics of a 'star.oddi' object.
   ...
   tmp <- aggregate(data.frame(n = x$year), by = x[vars], length)

   # Trim data:
   if (truncate) x <- truncate(x, ...)

   # Number of records per tow:
   res <- aggregate(data.frame(n_tow = x$year), by = x[vars], length)
   res$n <- tmp$n[match(res[vars], tmp[vars])]
   res <- res[c(vars, "n", "n_tow")]
      
   # Subset tow data:
   index <- match(y[vars], res[vars])  
   y <- y[which(!is.na(index)), ]

   # Calculate maximal depth:
   if (!("depth" %in% names(x))){ 
      res$depth <- aggregate(list(n = x$pressure), by = x[vars], median, na.rm = TRUE)
      x$depth <- x$pressure
   }else{
      res$depth <- aggregate(list(n = x$depth), by = x[vars], median, na.rm = TRUE)
   }
