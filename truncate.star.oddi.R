truncate.star.oddi <- function(x, buffer = 0, ...){
   # TRUNCATE.STAR.ODDI - Truncate a 'star.oddi' object.

   # Check input data:
   year <- unique(x$year)
   if (length(year) != 1) stop("'star.oddi' object 'x' must contain a single year of data.")
