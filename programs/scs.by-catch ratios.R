...
clg()
file <- paste0(getwd(), "/results/figures/english/scs.by-catch ratios.", min(years),  "-", max(years))
gdevice(format, file = file, height = 11, width = 8.5)
k <- kronecker(matrix(1:(2*length(species)), ncol = 2, byrow = TRUE), matrix(1, ncol = 5, nrow = 5))
...
for (j in 1:length(species)){
   ...
   y <- read.scscat(years, survey = "regular", species = species[j])
   if (species[j] == 2520) y <- read.scscat(years, survey = "regular", species = c(2520, 2521, 2527))
   if (species[j] == 6100) y <- read.scscat(years, survey = "regular", species = c(6100:6135, 6300:6399))
   y$tow.id <- tow.id(y)
   y <- aggregate(y["number.caught"], by = y[key(x)], sum)
   import(x, var = "number.caught", fill = 0) <- y
   x$n <- x$number.caught
   ...
   for (i in 1:length(years)){
      print(years[i])
      family <- nbinom1
      if ((species[j] == 1823) & (years[i] == 2016)) family <- nbinom2
   ...
   if (j == length(species)) axis(1, at = years[seq(1, length(years), by = 2)], las = 2)
   mtext(species(species[j]), 2, 2.5, cex = 1.0)
   ...
   axis(4)
   if (j == length(species)) axis(1, at = years[-length(years)], labels = paste0(years[-length(years)], "-", years[-1]), las = 2)
...
clg()
for (j in 1:length(species)){
   file <- paste0(getwd(), "/results/figures/english/maps/by-catch/catch ratios ", species(species[j]), ".", min(years),  "-", max(years))
   gdevice(format, file = file, height = 11, width = 8.5)
