...
clg()
file <- paste0(getwd(), "/results/figures/english/catch ratios/scs.by-catch ratios.", min(years),  "-", max(years))
...
for (j in 1:length(species)){
   ...
   if (species[j] == 6400) x$n[x$n > 1500] <- 0  # Heart urchin correction.
   ...
   if (j == 3) axis(1, las = 2, at = years, las = 2)
...
clg()
for (j in 1:length(species)){
   file <- paste0(getwd(), "/results/figures/english/catch ratios/catch ratios ", species(species[j]), ".", min(years),  "-", max(years))
   gdevice(format, file = file, height = 11, width = 8.5)
