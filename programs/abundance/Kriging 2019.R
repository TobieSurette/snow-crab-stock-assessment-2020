...
catch(s) <- catch(b, category = categories, weight = TRUE, hard.shelled = TRUE, units = "t")
s[categories] <- 1000000 * s[categories] / repvec(s$swept.area, ncol = length(categories))
...
ss <- summary(ss, category = categories, weight = TRUE, hard.shelled = TRUE, units = "t")
...
p <- locate.gulf.spatial(keywords = c("kriging", "poylgons"))
